...
const DEFAULT_SITES = [
  { id: 'youtube-shorts',    name: 'YouTube Shorts',    patterns: ['youtube.com/shorts/*'],   enabled: true,  isDefault: true },
  { id: 'youtube',           name: 'YouTube (all)',     patterns: ['youtube.com/*'],           enabled: false, isDefault: true },
  { id: 'reddit',            name: 'Reddit',            patterns: ['reddit.com/*'],            enabled: false, isDefault: true },
  { id: 'twitter',           name: 'X / Twitter',       patterns: ['twitter.com/*','x.com/*'], enabled: false, isDefault: true },
  { id: 'tiktok',            name: 'TikTok',            patterns: ['tiktok.com/*'],            enabled: false, isDefault: true },
  { id: 'instagram-reels',   name: 'Instagram Reels',   patterns: ['instagram.com/reels/*'],   enabled: false, isDefault: true },
  { id: 'instagram',         name: 'Instagram (all)',   patterns: ['instagram.com/*'],         enabled: false, isDefault: true },
  { id: 'facebook',          name: 'Facebook',          patterns: ['facebook.com/*'],          enabled: false, isDefault: true },
];
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    case 'HEARTBEAT':      handleHeartbeat(message.siteId, sendResponse); return true;
    case 'CHECK_STATUS':   checkStatus(message.siteId, sendResponse);     return true;
    case 'GET_STATE':      getState(sendResponse);                         return true;
    case 'GET_SITES':      getSites(sendResponse);                         return true;
    case 'UPDATE_SETTINGS': updateSettings(message.settings, sendResponse); return true;
    case 'UPDATE_SITES':   updateSites(message.sites, sendResponse);      return true;
    case 'RESET':          reset(message.siteId, sendResponse);           return true;
    case 'RESET_ALL':      resetAll(sendResponse);                        return true;
  }
});

async function getStorageData() {
  return chrome.storage.local.get(['sites', 'siteStates', 'watchLimit', 'cooldownDuration']);
}
...
async function reset(siteId, sendResponse) {
  const data = await chrome.storage.local.get(['siteStates']);
  const siteStates = data.siteStates || {};
  siteStates[siteId] = { watchedTime: 0, blockStartTime: null };
  await chrome.storage.local.set({ siteStates });
  sendResponse({ success: true });
